...
export class Slider {
  ...
  checkIntervalTimeout() {
    if (this.intervalTimeoutDuration) {
      ...
      if (this.dynamicIntervalTimeoutArray) {
        ...
        if (!hasSetInterval) {
          this.intervalTimeout = setInterval(this.intervalFunc, this.intervalTimeoutDuration);
        }
      } else {
        this.intervalTimeout = setInterval(this.intervalFunc, this.intervalTimeoutDuration);
      }
  ...
  componentWillLoad() {
    ...
    this.initialArr[this.activeSlideIndex ? (this.activeSlideIndex > 9 ? this.activeSlideIndex / 10 : this.activeSlideIndex) : 0] = true;
  ...
  handleClick(index: number) {
    ...
    this.stopTransitions = false;
    if (this.stopFirstAndLastSlideTransitions) {
      if ((idx === this.activeArr.length - 1 && index === 0) || (idx === 0 && index === this.activeArr.length - 1)) {
        this.stopTransitions = true;
      } else {
    ...
    for (let i = 0; i < this.initialArr.length; i++) {
      ...
      arr[i] = (
        <input
          ...
          name={`carousel-${this.sliderLang}-${this.path}-${this.radioButtonIdOffset ? this.radioButtonIdOffset : '0'}`}
          id={`carousel-${this.sliderLang}-${this.path}-${this.radioButtonIdOffset ? i + this.radioButtonIdOffset : i}`}
          {...inputProps}
        />
      );
    }
  ...
  renderLabelButtons = () => {
    ...
    return (
      <div class={`carousel-nav${this.theme ? ' dark-theme' : ''}${this.lastArrowBlack && this.activeArr.indexOf(true) === this.activeArr.length - 1 ? ' last-arrow-black' : ''}`}>
        <label
          htmlFor={
            this.activeArr.indexOf(true) + (this.radioButtonIdOffset ? this.radioButtonIdOffset : 0) > (this.radioButtonIdOffset ? this.radioButtonIdOffset : 0)
              ? `carousel-${this.sliderLang}-${this.path}-${
                  this.radioButtonIdOffset ? this.radioButtonIdOffset + this.activeArr.indexOf(true) - 1 : this.activeArr.indexOf(true) - 1
                }`
              : `carousel-${this.sliderLang}-${this.path}-${this.radioButtonIdOffset ? this.radioButtonIdOffset + this.activeArr.length - 1 : this.activeArr.length - 1}`
          }
          {...prevLabelProps}
        ></label>
        <label
          htmlFor={
            this.activeArr.indexOf(true) + (this.radioButtonIdOffset ? this.radioButtonIdOffset : 0) <
            (this.radioButtonIdOffset ? this.radioButtonIdOffset + this.activeArr.length - 1 : this.activeArr.length - 1)
              ? `carousel-${this.sliderLang}-${this.path}-${
                  this.radioButtonIdOffset ? this.radioButtonIdOffset + (this.activeArr.indexOf(true) + 1) : this.activeArr.indexOf(true) + 1
                }`
              : `carousel-${this.sliderLang}-${this.path}-${this.radioButtonIdOffset ? this.radioButtonIdOffset : 0}`
          }
          {...nextLabelProps}
        ></label>
      </div>
  ...
  render() {
    return (
      <div
        class={`css-carousel-${this.slides}${this.lastSlideWidthChecker()}${this.collapseOnMobile ? ' collapse-on-mobile' : ''}${
      ...
      >
        ...
        <div class="carousel-slides">
          <div class={`carousel-inner${this.carouselInnerTransitionFaster ? this.carouselInnerTransitionFaster : ''}`}>
            <slot />
          </div>
        </div>
      </div>
